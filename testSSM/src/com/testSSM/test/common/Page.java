...
public class Page {
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				...
				sb.append("	<a onclick=\"nextPage("
						+ (currentPage - 1)
						+ ");return false;\" href=\"javascript:void(0);\">上页</a>\n");
			}
			...
			if (currentPage == totalPage) {
				...
				sb.append("	<a onclick=\"nextPage("
						+ (currentPage + 1)
						+ ");return false;\" href=\"javascript:void(0);\">下页</a>\n");
				sb.append("	<a onclick=\"nextPage("
						+ totalPage
						+ ");return false;\" href=\"javascript:void(0);\">尾页</a>\n");
			}
			...
			sb.append("	<span>每页" + pageSize + "条记录,共" + totalResult
					+ "条</span>\n");
			sb.append("</div>\n");
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				...
				sb.append("	<a onclick=\""
						...
						+ "("
						+ (currentPage - 1)
						+ ");return false;\" href=\"javascript:void(0);\">上一页</a>\n");
			}
			...
			if (currentPage == totalPage) {
				...
				sb.append("	<a onclick=\""
						...
						+ "("
						+ (currentPage + 1)
						+ ");return false;\" href=\"javascript:void(0);\">下一页</a>\n");
			}
	...
	/**
	 * 获取简单版的分页组件，无form，非url提交； 包括 首页 上页 5 下页 尾页 共15页 每页.... type 判断是否显示 分页标签
	 */
		...
		if (totalResult > 0) {
			...
			if (currentPage == 1) {
				...
				if ("depotSearch".equals(type)) {
					sb.append("	<a onclick=\""
							+ nextPageMethod
							+ ",1);return false;\" href=\"javascript:void(0);\">首页</a>\n");
					sb.append("	<a onclick=\""
							+ nextPageMethod
							+ ","
							+ (currentPage - 1)
			...
			if (currentPage == totalPage) {
				...
				if ("depotSearch".equals(type)) {
					...
					sb.append("	<a onclick=\""
							...
							+ totalPage
							+ ");return false;\" href=\"javascript:void(0);\">尾页</a>\n");
					sb.append("	<span style='border:0px;padding:0px;'>到第<input id='txtPage"
							+ nextPageMethod
							+ "' type='text' size='3'/>页</span>\n");
					sb.append("	<label class='btn_gray_s' id = 'gotoNum' style='margin:0px;margin-bottom:4px;'><input type='button' value='GO' onclick='"
							+ nextPageMethod
							+ "(this.value);return false;'/></label>\n");
				}
