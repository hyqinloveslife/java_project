...
public class MatchInputDTO {
	...
	public Match convertToMatch(){
		MatchInputDTOConvert convert = new MatchInputDTOConvert();
		Match match = convert.converts(this);
		return match;
	}
	
	public MatchInputDTO convertFor(Match match){
//		MatchInputDTOConvert convert = new MatchInputDTOConvert();
//		MatchInputDTO dto = convert.doBackward(match);
//		return dto;
		throw new AssertionError("该方法不支持逆向调用");
	}
	
	private static class MatchInputDTOConvert implements DTOConverts<MatchInputDTO, Match>{
		...
		public MatchInputDTO doBackward(Match t) {
			...
			BeanUtils.copyProperties(t, matchInputDTO);
